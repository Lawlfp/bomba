...
<body>
    <div class="left-container">
		...
		<div style="width: 100%;
    height: auto;
    text-align: center;"><div style="width: 126px;height:29px;margin: 0 auto;background-color: #EB480B;border-radius:8px;box-shadow: 4px 4px 4px rgba(0, 0, 0, 0.25);"><a href="log.html"	 class="dashboard6" id="notactive"><img src="exit.png" class="logouticons"><span class="dashtextlogout">Log Out</span></a></div></div>
    </div>
    <div class="right-container">
        ...
        // Assuming patientid is passed via URL as a query parameter (e.g., treatments.php?patientid=123)
        $targetPatientId = isset($_GET['patientid']) ? (int)$_GET['patientid'] : 0;

        if ($targetPatientId > 0) {
            // Fetch treatment records for the specific patient
            $treatmentQuery = "SELECT `treatmentID`, `treatmentName`, `treatmentDate`, `patientid` FROM `treatment` WHERE `patientid` = $targetPatientId";
            ...
            echo "<p>Invalid or missing patient ID.</p>";
        }
        ...
        ?>
														...
														function deleteTreatment(treatmentId) {
															...
															xhr.send('treatment_id=' + treatmentId); // Send the treatment_id to the server
																						...
																						<form method="POST" action="submit_treatment.php?patientid=<?php echo $patientid;?>">
																						<div class="patientcontainer3">
																							<div class="flexcontainer1">
																								...
																								<button type="submit" style="z-index:99;font-family:Roboto;font-size:11px;font-weight:bold;color:#BEBBBB;margin-top:10px; margin-left:110px;background:none;outline:none;border:none;color:red;">Submit
																								</button>
